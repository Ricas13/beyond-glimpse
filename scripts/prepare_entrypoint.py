...
def prepare(path: Path) -> bool:
    ...
    source, jellyfin_changed = replace_one(source, (PREVIOUS_JELLYFIN, OLD_JELLYFIN), NEW_JELLYFIN)
    source, emby_changed = replace_one(source, (PREVIOUS_EMBY, OLD_EMBY), NEW_EMBY)
    changed = jellyfin_changed or emby_changed
    if changed:
